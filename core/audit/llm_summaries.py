...
def _parse_summary_response(
    ...
    return FunctionSummary(
        ...
        callees=[str(c) for c in callees[:20]],
        callers=[str(c) for c in callers[:20]],
        error_paths=[str(e) for e in error_paths[:10]],
        state_transitions=[str(s) for s in state_transitions[:10]],
        source="llm",
        confidence="medium",
        evidence_tier=EvidenceTier.HEURISTIC,
    )
