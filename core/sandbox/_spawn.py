    ...
    *,
    ...
    if audit_mode:
        ...
        if not seccomp_profile:
            ...
            audit_config = {
                ...
                # ``observe_nonce`` is a public run-correlation id.
                # The HMAC key remains in this config until the tracer
                # is ready; parent unlinks it before target exec. The
                # tracer signs data, budget markers, and its summary so
                # target-written JSONL cannot forge accepted evidence.
    ...
    try:
        ...
        if _audit_engaged:
            ...
            # config, including the HMAC key, before releasing target exec.
            if _audit_config_path is not None:
                try:
                    os.unlink(_audit_config_path)
                except FileNotFoundError:
