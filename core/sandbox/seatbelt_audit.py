...
def parse_log_entry(entry: dict, *,
    ...
    `nonce`: public run-correlation id. The parent uses it to select
    one run from a shared log; callers need an HMAC key to authenticate
    target-writable observe records.
    """
...
class LogStreamer:
    ...
    def __init__(self, run_dir: Path,
        ...
        # A nonce routes one run's records. The signer is the
        # authenticity boundary: it stays in the trusted streamer and
    ...
    def _append_record_locked(self, record: dict) -> None:
        ...
        signed = False
        try:
            if self._observe_signer is not None:
                self._observe_signer.sign(record)
                signed = True
            ...
            if signed:
                self._observe_signer.rollback(record)
            raise
...
def start_log_streamer(run_dir: Path,
    ...
    `observe_nonce` is a public correlation id. `observe_hmac_key` stays
    in the trusted parent/streamer and authenticates ordered records;
    pass that key to parse_observe_log(expected_hmac_key=...).
    """
    s = LogStreamer(
        run_dir,
