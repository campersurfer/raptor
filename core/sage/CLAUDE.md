...
| `recall_proven_rules` / `store_proven_rule_metadata` | Accumulate proven checker rules; recall by engine+CWE for replay. Shared by `/agentic` and `/audit` | `raptor-rule-library` |
...
- **When auditing:** hypothesis verdicts stored at commit time; tool-confirmed observations stored to methodology domain; proven rules shared with `/agentic` via `raptor-rule-library`.
